...
const catalog = catalogUrl.startsWith('file:')
  ...
  : await fetch(catalogUrl).then((response) => response.ok ? response.json() : Promise.reject(new Error('Catalog request failed: ' + response.status)));
...
const status = (passport, key) => {
  ...
  return value === 'verified' ? 'VERIFIED' : value === 'detected' ? 'DETECTED' : 'UNKNOWN';
};

const featured = projects.map((project, index) => {
...
const colorFor = (value) => value === 'VERIFIED' ? '#3fb950' : value === 'DETECTED' ? '#d29922' : '#8b949e';
